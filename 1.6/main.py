...
X = 8000*scale
Y = 8000*scale
scrn = pygame.display.set_mode((X, Y), pygame.FULLSCREEN | pygame.SCALED | pygame.HWSURFACE)
pygame.init()
undercheck_img = pygame.transform.smoothscale(pygame.image.load(PATH+'undercheck.png').convert_alpha(),(990*scale,990*scale))
board_img = pygame.transform.smoothscale(pygame.image.load(PATH+'board.png').convert(),(8000*scale,8000*scale))
circle_img = pygame.transform.smoothscale(pygame.image.load(PATH+'circle.png').convert_alpha(),(300*scale,300*scale))
selected_img = pygame.transform.smoothscale(pygame.image.load(PATH+'selected.png').convert_alpha(),(990*scale,990*scale))
debag_img = pygame.transform.smoothscale(pygame.image.load(PATH+'debag.png').convert_alpha(),(979.59*scale,979.59*scale))
HD_img = pygame.image.load(PATH+'HD.png').convert_alpha()
def get_position_on_HD(row, column):
    height = HD_img.get_height()
    width = HD_img.get_width()
    return HD_img.subsurface(pygame.Rect( width//6*(row-1), height//2*(column-1),(width//6),(height//2)))
pieces = {'bP': pygame.transform.smoothscale(get_position_on_HD(6,2),(979.59*scale,979.59*scale)),
          'bfP': pygame.transform.smoothscale(pygame.image.load(PATH+'bfP.png').convert_alpha(),(979.59*scale,979.59*scale)),
          'wfP': pygame.transform.smoothscale(pygame.image.load(PATH+'wfP.png').convert_alpha(),(979.59*scale,979.59*scale)),
          'bk': pygame.transform.smoothscale(get_position_on_HD(4,2),(979.59*scale,979.59*scale)),
          'bB': pygame.transform.smoothscale(get_position_on_HD(3,2),(979.59*scale,979.59*scale)),
          'bR': pygame.transform.smoothscale(get_position_on_HD(5,2),(979.59*scale,979.59*scale)),
          'bQ': pygame.transform.smoothscale(get_position_on_HD(2,2),(979.59*scale,979.59*scale)),
          'bK': pygame.transform.smoothscale(get_position_on_HD(1,2),(979.59*scale,979.59*scale)),
          'wP': pygame.transform.smoothscale(get_position_on_HD(6,1),(979.59*scale,979.59*scale)),
          'wk': pygame.transform.smoothscale(get_position_on_HD(4,1),(979.59*scale,979.59*scale)),
          'wB': pygame.transform.smoothscale(get_position_on_HD(3,1),(979.59*scale,979.59*scale)),
          'wR': pygame.transform.smoothscale(get_position_on_HD(5,1),(979.59*scale,979.59*scale)),
          'wQ': pygame.transform.smoothscale(get_position_on_HD(2,1),(979.59*scale,979.59*scale)),
          'wK': pygame.transform.smoothscale(get_position_on_HD(1,1),(979.59*scale,979.59*scale)),
...
class board():
    ...
    def gui_print(self):
            ...
            for letter in letters:
                for i in range(0,8):
                    ...
                    if piece != None:
                        ...
                        if piece[1] == 'K':
                            if self.get_board_cell_info([letter, i+1]).is_check:
                                scrn.blit(undercheck_img,((letters.index(letter)%8)*(END_X-START_X)/8+START_X, END_Y-(END_Y-START_Y)/8-(i)*(END_Y-START_Y)/8))
        ...
        for move in self.moves:
            scrn.blit(circle_img,((letters.index(move[0])%8)*(END_X-START_X)//8+START_X+((979.59-300)/2)*scale, END_Y-(END_Y-START_Y)//8-(move[1]-2)*(END_Y-START_Y)//8-(979.59+300)/2*scale))
        pygame.display.flip()
    ...
    def get_board_cell_info(self, cordinates):
        letters = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']
        if cordinates[1] <= 0 or cordinates[1]-1 > 7:
        	return None
                ...
                if self.get_board_cell_info([letter, num]) != None:
                    if self.get_board_cell_info([letter, num]).color == color:
                       for move in attacking_moves(self.get_board_cell_info([letter, num])):
                            if move not in self.attacking_map:
                               self.attacking_map.append(move)
        return self.attacking_map
